...
@app.route('/index')
def index():
	# items = Diginadmin(amail='sneha.savarkar@spit.ac.in', apassword='sneha123')
...
@app.route('/search')
def search():
	items = Items.query.whoosh_search(request.args.get('query')).all()
	restadmin = items.shop

	return render_template('index.html',restadmin =restadmin )
